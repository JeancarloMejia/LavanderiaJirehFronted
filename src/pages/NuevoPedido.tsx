...
import { Plus, Trash2, ArrowLeft, Package, ChevronDown } from "lucide-react";
...
const INPUT_CLASS =
  "w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black px-3 py-2 text-sm text-black dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent";
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 mt-1 w-full rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-black shadow-lg overflow-hidden">
          {filtered.map((o) => (
            <button
              key={o.id}
              type="button"
              className="w-full text-left px-3 py-2 text-sm text-black dark:text-white hover:bg-violet-50 dark:hover:bg-violet-500/10 flex items-center justify-between"
              onClick={() => {
                setQuery(o.label);
                onChange(o.id);
                setOpen(false);
            ...
            >
              <span>{o.label}</span>
              {o.extra && <span className="text-xs text-slate-500 dark:text-slate-400">{o.extra}</span>}
            </button>
          ))}
        </div>
      )}
...
export function NuevoPedido() {
  ...
  const clienteOptions: Option[] =
    clientes?.results.map((c) => ({ id: c.id, label: `${c.nombres} ${c.apellidos}` })) ?? [];

  const empleadoOptions: Option[] =
    empleados?.results.map((e) => ({ id: e.id, label: `${e.nombres} ${e.apellidos}` })) ?? [];

  const servicioOptions: Option[] =
    servicios?.results.map((s) => ({
      id: s.id,
      label: s.nombre_servicio,
      extra: `S/ ${parseFloat(s.precio_base).toFixed(2)}/kg`,
    })) ?? [];
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-black dark:text-white">Nuevo pedido</h1>
      ...
      <form onSubmit={handleSubmit((d) => crear.mutate(d))} className="space-y-5">
        <Card>
          <CardHeader>
            <h2 className="text-sm font-semibold text-black dark:text-white">Datos generales</h2>
          </CardHeader>
          <CardBody className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-black dark:text-white">Cliente</label>
                <SearchableSelect
                  options={clienteOptions}
                  value={undefined}
                  onChange={(id) => setValue("id_cliente", id ?? 0)}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-black dark:text-white">Empleado asignado</label>
                <SearchableSelect
                  options={empleadoOptions}
                  value={undefined}
                  onChange={(id) => setValue("id_empleado", id ?? 0)}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input label="Fecha de entrega estimada" type="datetime-local" {...register("fecha_entrega")} />
              <Textarea label="Observaciones" rows={1} placeholder="Notas adicionales..." {...register("observaciones")} />
            </div>
          </CardBody>
        </Card>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-black dark:text-white">Prendas</h2>
              </div>
              <Button
                type="button"
                variant="secondary"
          ...
          <CardBody className="space-y-4">
            {errors.prendas?.root && <p className="text-xs text-red-400">{errors.prendas.root.message}</p>}
            ...
            {fields.map((field, i) => {
              ...
              return (
                <div key={field.id} className="border border-white/8 rounded-xl p-4 space-y-3 bg-white/3">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {precio !== null && (
                        <span className="text-sm font-bold text-violet-400">S/ {precio.toFixed(2)}</span>
                      )}
                      {fields.length > 1 && (
                        <button
                          type="button"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    />
                    <Input label="Color" placeholder="Ej: Azul, Blanco..." {...register(`prendas.${i}.color`)} />
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    ...
                    />
                    <Input label="Observaciones" placeholder="Opcional..." {...register(`prendas.${i}.observaciones`)} />
                  </div>

                  <div className="flex flex-col gap-1.5">
                    <label className="text-sm font-medium text-black dark:text-white">Servicio</label>
                    <SearchableSelect
                      options={servicioOptions}
                      value={undefined}
                      onChange={(id) => setValue(`prendas.${i}.id_servicio`, id)}
